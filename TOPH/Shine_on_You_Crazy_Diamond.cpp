...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int d = 1; d * d <= temp; ++d) {
            if(temp % d == 0) {
                mp[d]++;
                if(d * d != temp) mp[temp/d]++;
            }
        }
    }   
    vector<pair<int, int>> vpr;
    for (int i = 0; i < pr.size(); ++i) {
        vpr.push_back({mp[pr[i]], pr[i]});
    }
    sort(all(vpr));
    reverse(all(vpr));
    int res = 0, tempRes = 0, p1 = 2, p2  = 3;
    for (int i = 0; i < pr.size(); ++i) {
        for (int j = i + 1; j < pr.size(); ++j) {
            tempRes = vpr[i].ff + vpr[j].ff;
            if(tempRes <= res) break;       // MOST IMPORTANT LINE OF THE CODE
            if(vpr[i].ss * vpr[j].ss < N) 
                tempRes -= mp[vpr[i].ss * vpr[j].ss];

            if(tempRes > res) {
                res = tempRes;
                p1 = vpr[i].ss, p2 = vpr[j].ss;
            }
        }
    }
    // bug(mp[2]);
