...
void tTestCase(int t) {
    int n, k;
    scan(n, k);
    priority_queue<int> cats, mouse;

    for (int i = 0; i < n; ++i) {
        char temp; scan(temp);
        if(temp == 'C') cats.push(i);
        else mouse.push(i);
    }
    int ans = 0;
    while(cats.size() and mouse.size()) {
        if(abs(cats.top() - mouse.top()) > k) {
            if(cats.top() > mouse.top()) cats.pop();
            else mouse.pop();
        } else {
            ans++;
            cats.pop();
