...
void tTestCase(int t) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if(j != 0) {
                if(mtx[i][j] == mtx[i][j - 1]) res[mtx[i][j]]++;
            }
            if(j != m - 1) {
                if(mtx[i][j] == mtx[i][j + 1]) res[mtx[i][j]]++;
            }
            ...
            if(i != 0) {
                if(mtx[i][j] == mtx[i - 1][j]) res[mtx[i][j]]++;
    ...
    int cnt = 0;
    bool flag = false;
    int totalOp = st.size(), twoOP =  res.size();
    int oneOP = totalOp - twoOP;
    int ans = oneOP + (2 * twoOP);
    if(twoOP) ans -= 2;
    else if(oneOP) ans--;
    // print(oneOP, twoOP);
    print(ans);
    // print(st.size(), res.size());
