...
bool comp(tuple<string, int, vector<int>> a, tuple<string, int, vector<int>> b) {
    if (get<1>(a) != get<1>(b)) {
...
bool comp2(tuple<string, int, vector<int>> a, tuple<string, int, vector<int>> b) {
    for (int i = 0; i < 60; ++i) {
        if (get<2>(a)[i] != get<2>(b)[i]) {
            return get<2>(a)[i] >= get<2>(b)[i];
        }
...
vi scoreBoard = {25, 18, 15, 12, 10, 8, 6, 4, 2, 1};
void solve() {
    scoreBoard.resize(70);
    int t; scan(t);
    vector<tuple<string, int, vector<int>>> vt;
    map<string, int> mp;
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; ++i) {
            string s; scan(s);
            if(position.find(s) == position.end()) {
                position[s].resize(60, 0);
            }
            position[s][i]++;
            mp[s] += scoreBoard[i];
        }
    ...
    for(auto i : mp) {
        vt.push_back({i.ff, i.ss, position[i.ff]});
    }
    sort(all(vt), comp);
    print(get<0>(vt[0]));
    sort(all(vt), comp2);
