...
int slDist(int x1, int y1, int x2, int y2) {
...
void tTestCase(int t) {
    ...
    for (int i = 0; i < n; ++i) {
        int x, y; scan(x, y); vpr.push_back({x, y});
    }
    int xs, ys, xt, yt; scan(xs, ys, xt, yt);
    int mxDist = slDist(xs, ys, xt, yt);
    for (int i = 0; i < n; ++i) {
        if(slDist(xt, yt, vpr[i].ff, vpr[i].ss) <= slDist(xt, yt, xs, ys)) {
            no; return;
        }
    }
    yes;
