...
int f(vi &res, int a, int b) {
    if(a == b) return 1;
    if(a > b) return 0;

    // return f(res, a * 2, b) or f(res, (10*a) + 1, b);
    bool check = 0;
    a *= 2;
    res.push_back(a);
    check = f(res, a, b);
...
// for f2 reverse the array and no need to print first element separately
// int f2(vi &res, int a, int b) {
//     if(a == b) {res.push_back(a); return 1;}
//     if(a > b) return 0;
