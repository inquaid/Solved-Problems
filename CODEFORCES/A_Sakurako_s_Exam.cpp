...
int f(int a, int b, int sum) {
    if(a == 0 and b == 0) return sum == 0;
    if(dp[a][b][sum] != -1) return dp[a][b][sum];
    if(a == 0) {
        return dp[a][b][sum] = f(a, b - 1, sum + 2) or f(a, b - 1, sum - 2);
    } else if(b == 0) {
        return dp[a][b][sum] = f(a - 1, b, sum + 1) or f(a - 1, b, sum - 1);
    } else {
        return dp[a][b][sum] = f(a - 1, b, sum + 1) or f(a - 1, b, sum - 1) or f(a, b - 1, sum + 2) or f(a, b - 1, sum - 2);
    }
}
